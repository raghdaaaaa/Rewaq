...
export class RegisterComponent {
  ...
  // Keeps focus and caret where they were: switching an input's type
  // makes the browser drop focus and jump the cursor to the end.
    ...
    setTimeout(() => {
      ...
      if (caret !== null) {
        try {
          ...
          /* some browsers refuse setSelectionRange on certain types */
        }
      }
    });
  }
  ...
  onSubmit() {
    ...
    const payload = {
      ...
      role: 'user', // New public registrations are strictly patrons
    };
    ...
    this.authService.register(payload).subscribe({
      next: (response: any) => {
        ...
        if (response.token) {
          ...
          // Always route newly registered users to their user dashboard
