...
export class DashboardComponent implements OnInit {
  ...
  // Active view tab: 'inventory' | 'users' | 'browse' | 'my-borrowings' | 'book-details'
  ...
  // Results from GET /books/search. Kept separate from books() so the
  // inventory stat cards keep counting the whole catalogue during a search.
  ...
  // Profile form — no role field: you cannot change your own permissions
  ...
  filteredBooks = computed(() => {
    // While searching, show what the backend matched; otherwise the full list
  ...
  // Computed Stats for Inventory (matching Image 1)
  stats = computed(() => {
  ...
  setTab(tab: 'inventory' | 'users' | 'browse' | 'my-borrowings' | 'book-details'): void {
    // Strict protection: Non-admin users cannot switch to admin tabs
  ...
  // ==========================================
  // Book Details Navigation (Image 3)
  // ==========================================
  ...
  onBookSearch(query: string): void {
    ...
    // An empty term would make the backend regex fail, so just show everything
    if (!term) {
    ...
    // wait for a pause in typing instead of calling the API on every key
  ...
  openProfileModal(): void {
    ...
    // GET /users/me is the only place the phone number comes from,
    // the login token only carries name, email and role
  ...
  submitProfile(): void {
    ...
    this.userService.updateMe(data).subscribe({
      next: (updated) => {
        ...
        // refresh the stored user so the navbar name updates straight away
